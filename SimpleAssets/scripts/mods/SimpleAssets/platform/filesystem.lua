...
if not pcall(ffi.typeof, "SimpleAssets_FILESYSTEM_CDEF") then
	ffi.cdef([[
		typedef struct { int unused; } SimpleAssets_FILESYSTEM_CDEF;

		SimpleAssets_HANDLE FindFirstFileW(const SimpleAssets_WCHAR* lpFileName, SimpleAssets_WIN32_FIND_DATAW* lpFindFileData);
		SimpleAssets_BOOL FindNextFileW(SimpleAssets_HANDLE hFindFile, SimpleAssets_WIN32_FIND_DATAW* lpFindFileData);
		SimpleAssets_BOOL FindClose(SimpleAssets_HANDLE hFindFile);
	]])
end
...
local function find_entries(search_pattern)
	...
	if windows.kernel32.FindClose(handle) == 0 then
		error(string.format("Failed to close asset file listing handle: Windows error %d", windows.last_error()))
	end
...
local function collect_files(directory_path, relative_base_path, recursive, files)
	local entries = find_entries(join_path(directory_path, "*"))
	...
	for i = 1, #entries do
		...
		if entry.is_directory then
			if recursive then
				collect_files(child_path, relative_path, recursive, files)
			end
		else
...
filesystem.list_files = function(directory_path, recursive)
	...
	if not windows.is_directory(directory_path) then
		error(string.format("Asset directory was not found: %s", directory_path))
	end

	local files = {}

	collect_files(directory_path, "", recursive == true, files)
	table.sort(files)
